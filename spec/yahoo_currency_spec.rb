...
describe "YahooCurrency" do
  ...
  describe "#get_yahoo_rate" do
    it "should work for USD->USD" do
      @bank.get_yahoo_rate('USD', 'USD').should == 1.0
    end
    ...
    it "should work for Currency.wrap(:USD)->Currency.wrap(:USD)" do
      @bank.get_yahoo_rate(Money::Currency.wrap(:USD),
                            Money::Currency.wrap(:USD)).should == 1.0
    end

    it "should raise an UnknownCurrency error for an unknown currency" do
      lambda{@bank.get_yahoo_rate('USD', 'BATMAN')}.should raise_error(Money::Currency::UnknownCurrency)
    end

    it "should raise and UnknownRate error for a known currency but unknown rate" do
      lambda{@bank.get_yahoo_rate('USD', 'ALL')}.should raise_error(Money::Bank::UnknownRate)
    end
  end

  describe "#get_rate" do
    it "should use #get_yahoo_rate when rate is unknown" do
      @bank.should_receive(:get_yahoo_rate).once
      @bank.get_rate('USD', 'USD')
    end

    it "should not use #get_yahoo_rate when rate is known" do
      @bank.get_rate('USD', 'USD')
      @bank.should_not_receive(:get_yahoo_rate)
      @bank.get_rate('USD', 'USD')
    end
